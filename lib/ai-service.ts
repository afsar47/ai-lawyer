...
export class AIService {
  ...
  async generateText(request: AIRequest): Promise<AIResponse> {
    try {
      ...
      // Enterprise-safe execution: server loads provider + apiKey from DB.
      ...
      if (!response.ok) {
        throw new Error(data?.error || `AI API error: ${response.statusText}`);
      }

      const responseTime = Date.now() - startTime;
      const content = String(data?.content || '');
      const tokensUsed = this.estimateTokens(request.prompt + content);
      const cost = 0; // cost is tracked server-side via model config; keep 0 client-side for now

      return {
        success: true,
        content,
        modelUsed: data?.model?.name || data?.model?.id || 'unknown',
        tokensUsed,
        responseTime,
        cost
      };
