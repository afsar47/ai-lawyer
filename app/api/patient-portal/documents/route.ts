...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!clientId) return NextResponse.json({ documents: [], total: 0 });

    const { searchParams } = new URL(request.url);
    const matterId = searchParams.get('matterId');

    const query: Record<string, any> = { clientId, sharedWithClient: true };
    ...
    console.error('Client portal documents fetch error:', error);
