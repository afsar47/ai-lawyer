...
export default function ClientPortalMattersPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch('/api/patient-portal/matters');
    ...
    <div className="space-y-6">
      <div className="flex items-center space-x-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600">View cases you are a party to.</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search by case #, title, practice area..."
        ...
        {loading ? (
          ...
          <div className="divide-y divide-gray-100">
            {filtered.map((m) => (
              <Link
                key={m._id}
                href={`/patient-portal/matters/${m._id}`}
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xs text-gray-500">
                      {m.practiceArea || '—'}{m.createdAt ? ` • Opened ${new Date(m.createdAt).toLocaleDateString()}` : ''}
                    </div>
                  </div>
